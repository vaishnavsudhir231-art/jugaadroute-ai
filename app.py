...
st.markdown('<p style="text-align: center; color: #10B981; font-weight: 600; letter-spacing: 0.5px;">PREMIUM SMART HYBRID ENGINE (v23.0)</p>', unsafe_allow_html=True)
st.write("---")
...
# 🔐 मुख्य स्क्रीन पर ही सबसे ऊपर प्रीमियम सेटिंग्स ड्रॉपडाउन
...
if st.button("🔥 एआई वन-क्लिक MASTEROUT डिकोड करो", use_container_width=True):
    if not origin_input or not dest_input:
        ...
        # 💎 लेग डिटेल्स का प्रीमियम लेआउट (अब कड़क ब्लैक टेक्स्ट के साथ)
        c1, c2 = st.columns(2)
        with c1:
            st.markdown(f"<div style='background-color: #F3F4F6; color: #111827; padding: 12px; border-radius: 8px; border-left: 5px solid #3B82F6;'><b>📍 सेक्टर १ (Leg 1)</b><br>{src} ➔ JAIPUR (JP)</div>", unsafe_allow_html=True)
            st.caption("💺 श्रेणी: Sleeper (SL) | **उपलब्धता: २४ सीटें**")
        with c2:
            st.markdown(f"<div style='background-color: #F3F4F6; color: #111827; padding: 12px; border-radius: 8px; border-left: 5px solid #10B981;'><b>🏁 सेक्टर २ (Leg 2)</b><br>JAIPUR (JP) ➔ {dest}</div>", unsafe_allow_html=True)
